...
impl SqlFinder {
    pub(super) fn analyze_assignment(
        ...
        assign.targets.iter().enumerate().for_each(|(i, target)| {
            ...
            debug!(
                "Assignment target {} yielded {} SQL results",
                i + 1,
                sql_results.len()
            );
            ...
            let sql_strings: Vec<SqlString> = sql_results
                ...
                .map(|result| {
                    ...
                    debug!(
                        "Converted SQL result to string: variable='{}', content preview='{}'",
                        sql_string.variable_name,
                        &sql_string.trunc_default()
                    );
                    sql_string
                })
                .collect();
        ...
        debug!(
            "Assignment analysis complete: found {} SQL strings total",
            results.len()
        );
        results
    }
    ...
    pub(super) fn analyze_stmt_expr(
        ...
        let sql_strings: Vec<SqlString> = results
            ...
            .map(|result| {
                let sql_string = sql_result_to_string(result, range_file);
                debug!(
                    "Expression converted to SQL string: variable='{}', content preview='{}'",
                    sql_string.variable_name,
                    sql_string.trunc_default()
                );
                sql_string
        ...
        debug!(
            "Expression statement analysis complete: {} SQL strings",
            sql_strings.len()
        );
        sql_strings
    }
    ...
    pub(super) fn analyze_annotated_assignment(
        ...
        if let Some(val) = &assign.value {
            ...
            debug!(
                "Annotated assignment yielded {} SQL results",
                sql_results.len()
            );
            ...
            let sql_strings: Vec<SqlString> = sql_results
                ...
                .map(|result| {
                    let sql_string = sql_result_to_string(result, range_file);
                    debug!("Annotated assignment converted to SQL string: variable='{}', content preview='{}'", sql_string.variable_name, sql_string.trunc_default());
                    sql_string
                })
                .collect();
        ...
        debug!(
            "Annotated assignment analysis complete: {} SQL strings",
            results.len()
        );
        results
    }
...
impl SqlFinder {
    ...
    fn process_call_expr(&self, call: &ast::ExprCall) -> Vec<SqlResult> {
        ...
        if !self.config.is_sql_function_name(&function_name) {
            debug!("Function '{function_name}' is not configured as SQL function, skipping");
            return vec![];
        }

        debug!(
            "Function '{function_name}' is configured as SQL function, processing {} args and {} keywords",
            call.args.len(),
            call.keywords.len()
        );

        let keyword_results: Vec<SqlResult> =
            call.keywords
                .iter()
                .enumerate()
                .flat_map(|(i, keyword)| {
                    debug!(
                        "Processing keyword argument {}/{}",
                        i + 1,
                        call.keywords.len()
                    );
                ...
                |arg_name| {
                    debug!("Keyword argument {}: name='{arg_name}'", i + 1);
                    if self.config.is_sql_class_name(arg_name) {
                        debug!("Keyword '{arg_name}' matches SQL class name, extracting content");
                        self.extract_content_flattened(&keyword.value, &function_name)
                    } else {
                        debug!("Keyword '{arg_name}' does not match SQL class name, skipping");
                        vec![]
                    }
                }
            )
        ...
        debug!(
            "Keyword arguments yielded {} SQL results",
            keyword_results.len()
        );
        ...
        let arg_results: Vec<SqlResult> = call
            ...
            .flat_map(|(i, arg)| {
                debug!(
                    "Processing positional argument {}/{}",
                    i + 1,
                    call.args.len()
                );
        ...
        debug!(
            "Positional arguments yielded {} SQL results",
            arg_results.len()
        );
        ...
        debug!(
            "Function call '{function_name}' yielded {} total SQL results",
            total_results.len()
        );
    ...
    fn extract_content_flattened(&self, expr: &ast::Expr, variable_name: &str) -> Vec<SqlResult> {
        debug!(
            "Extracting content from expression for variable '{variable_name}' at range {:?}",
            expr.range()
        );
        ...
        match expr {
            ...
            ast::Expr::BoolOp(ast::ExprBoolOp { values, .. }) => {
                debug!(
                    "Expression is a boolean operation with {} values",
                    values.len()
                );
                values
                    .iter()
                    ...
                    || *op == Operator::Div =>
            {
                ...
                self.extract_from_bin_op(bin).map_or_else(
                    || {
                    ...
                    |content| {
                        debug!(
                            "Binary operation yielded content: '{}'",
                            content.trunc_default()
                        );
                        vec![SqlResult {
            ...
            _ => {
                ...
                self.extract_content(expr).map_or_else(
                    ...
                    |content| {
                        debug!(
                            "Direct content extraction succeeded: '{}'",
                            content.trunc_default()
                        );
                        vec![SqlResult {
                            byte_range: expr.range().into(),
    ...
    fn process_assignment_target(&self, target: &ast::Expr, value: &ast::Expr) -> Vec<SqlResult> {
        ...
        match target {
            ...
            ast::Expr::Tuple(tuple) => {
                debug!(
                    "Assignment target is a tuple with {} elements",
                    tuple.elts.len()
                );
            ...
            ast::Expr::List(list) => {
                debug!(
                    "Assignment target is a list with {} elements",
                    list.elts.len()
                );
    ...
    fn process_by_ident(&self, name: &Identifier, value: &ast::Expr) -> Vec<SqlResult> {
        ...
        if self.config.is_sql_variable_name(name) {
            debug!(
                "Identifier '{}' matches SQL variable name, extracting content",
                name
            );
        ...
        debug!(
            "Identifier '{}' does not match SQL variable name, skipping",
            name
        );
        vec![]
    }
    ...
    fn handle_tuple_assignment(&self, targets: &[ast::Expr], value: &ast::Expr) -> Vec<SqlResult> {
        ...
        match value {
            ast::Expr::Tuple(tuple_value) => {
                debug!(
                    "Tuple assignment value is also a tuple with {} elements",
                    tuple_value.elts.len()
                );
                self.process_paired_assignments(targets, &tuple_value.elts)
            }
            ast::Expr::List(list_value) => {
                debug!(
                    "Tuple assignment value is a list with {} elements",
                    list_value.elts.len()
                );
    ...
    fn process_paired_assignments(
        ...
        debug!(
            "Processing paired assignments: {} targets, {} values",
            targets.len(),
            values.len()
        );
        ...
        for (target_idx, target) in targets.iter().enumerate() {
            ...
            }) = target
            {
                ...
                let target_results = self.process_assignment_target(starred_target, &new_list_expr);
                ...
                debug!(
                    "Processing regular target with value at index {}",
                    value_idx
                );
        ...
        debug!(
            "Paired assignments complete: {} total results",
            results.len()
        );
        results
    }
    ...
    fn target_contains_sql_variable(&self, target: &ast::Expr) -> bool {
        let contains_sql = match target {
            ...
            ast::Expr::Attribute(att) => {
                ...
                debug!(
                    "Checking if attribute '{}' is SQL variable: {}",
                    att.attr, is_sql
                );
                is_sql
            }
            ast::Expr::Tuple(tuple) => {
                debug!(
                    "Checking tuple with {} elements for SQL variables",
                    tuple.elts.len()
                );
                tuple
                    .elts
            ...
            ast::Expr::List(list) => {
                debug!(
                    "Checking list with {} elements for SQL variables",
                    list.elts.len()
                );
                list.elts
                    .iter()
            ...
            _ => {
                debug!(
                    "Target type does not contain SQL variables: {:?}",
                    std::mem::discriminant(target)
                );
                false
            }
...
impl SqlFinder {
    ...
    fn extract_content(&self, expr: &ast::Expr) -> Option<FinderType> {
        debug!(
            "Extracting content from expression type: {:?}",
        ...
        let result = match expr {
            ...
            ast::Expr::JoinedStr(j) => {
                debug!(
                    "Expression is a joined string (f-string) with {} parts",
                    j.values.len()
                );
                ...
                parts.map(|parts| {
                    ...
                    debug!(
                        "F-string combined result: '{}'",
                        &combined[..combined.len().min(50)]
                    );
        ...
        match &v.op {
            ast::Operator::Mod => {
                ...
                let (args, kwargs) = match &*v.right {
                    ...
                    ast::Expr::Dict(d) => {
                        debug!(
                            "Right side is a dictionary with {} key-value pairs",
                            d.keys.len()
                        );
                ...
                match expr_content {
                    FinderType::Str(fmt_string) => {
                        debug!(
                            "Formatting string with {} positional and {} keyword args",
                            args.len(),
                            kwargs.len()
                        );
                        let result =
                        ...
                        if let Some(ref formatted) = result {
                            debug!(
                                "String formatting successful: '{}'",
                                formatted.trunc_default()
                            );
                        } else {
    ...
    fn extract_arithmetic(
        ...
        if let Some(ref res) = result {
            debug!("Arithmetic operation successful: '{}'", res.trunc_default());
        } else {
    ...
    fn extract_format_call(
        ...
        debug!(
            "Processing string.format() call with {} args and {} kwargs",
            args.len(),
            kwargs.len()
        );
        ...
        for (i, a) in args.iter().enumerate() {
            debug!(
                "Processing format() positional argument {}/{}",
                i + 1,
                args.len()
            );
        ...
        for (i, kw) in kwargs.iter().enumerate() {
            debug!(
                "Processing format() keyword argument {}/{}",
                i + 1,
                kwargs.len()
            );
        ...
        debug!(
            "Extracted {} keyword format arguments, has_unpacked_dict: {}",
            kw_fills.len(),
            has_unpacked_dict
        );
        ...
        if has_unpacked_dict {
            ...
            result = numbered_re
                .replace_all(&result, |caps: &regex::Captures| {
                    ...
                    debug!(
                        "Replacing numbered placeholder {} with positional arg",
                        index
                    );
                    if index < pos_fills.len() {
                        ...
                        warn!(
                            "Format placeholder index {} out of range (have {} args)",
                            index,
                            pos_fills.len()
                        );
